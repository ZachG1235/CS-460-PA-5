/*
 * StringUtils.c belong to Michael Leverington but have been re-written 
...
void concatenateString( char *destStr, const char *sourceStr)
{
    ...
    while (tempStr[sourceIndex] != NULL_CHAR && destIndex < MAX_STR_LEN)
    {
        ...
        // set temporary end of desintation string
        destStr[destIndex] = NULL_CHAR;
    }
    // end loop

    // release memeory user for temp string
        // function: free
    free(tempStr);
}
...
int findSubString(const char *testStr, const char *searchSubStr)
{
    ...
    int testStrLen = getStringLength(testStr);
        ...
        // init master index- location of sub string start point
    int masterIndex = 0;
    ...
    // assume have failed at this point, return SUBSTRING_NOT_FOUND
...
{
    // call engine function with specified constrains
                            ...
                            int startIndex, int endIndex)
{
    ...
    int sourceStrLen = getStringLength(sourceStr);
        ...
        // init the desintation index to 0
    int destIndex = 0;

        // init source index to start index (parameter)
                                            ...
                                            && endIndex < sourceStrLen)
    {
        ...
        {
            // assign source character to desitnation elements
            destStr[destIndex] = tempStr[sourceIndex];
